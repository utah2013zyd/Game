...
void FCGraph::AdjacentCost( void* pos, std::vector< micropather::StateCost > *neighbors )
{
	...
	for( int i=0; i<8; ++i ) {
			 ...
			 && ny >= 0 && ny < _height )
		{
			if(cost[i] == 1.0f)
			{
				micropather::StateCost nodeCost = { FCPather::XY2pos( nx, ny , _width, _height), cost[i]* _graph.at(ny*_width + nx).getValue() };
			...
			//{
			//	// Normal floor
			//	StateCost nodeCost = { XY2pos( nx, ny ), FLT_MAX };
			//	neighbors->push_back( nodeCost );
			//}
		//}
	}
...
void FCGraph::addBlocker(int x, int y, Orz::EventHandler * blocker)
{
 ...
 Orz::EventHandler * FCGraph::getBlocker(int x, int y) const
{
	return _graph.at(y*_width + x).getBlocker();
}
