...
#include "FCEvents.h"
using namespace Orz;
FCWall::FCWall(const std::string & name, int pos):FCObject(name),_pos(pos)
{
...
void FCWall::doExecute(Event * evt)
{
	...
	if(FCEvents::equal(evt, FCEvents::SHOOTING))
	{
		...
		if(map)
		{
			int x = _pos/16;
			int y = _pos%16;
			map->removeBlocker(x, y, this);
			map->setWay(x, y, FCPather::WAY, 1.f);
		}
	}
}
//这个函数会在初始化调用
