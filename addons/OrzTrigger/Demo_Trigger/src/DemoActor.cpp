...
void DemoActor::doEnable(void)
{
   ...
   _animState->setEnabled(true);
	...
	_trigger.reset(new Trigger(12));
	//;
	//eh->enableUpdate();
	_trigger->addEventHandler(TriggerEventHandlerPtr(new TriggerEventHandlerPolling()), getWorld());
	_trigger->addConditions(TriggerConditionsPtr(new OgreRangeTriggerConditions(Ogre::Vector3(0,0,100), headNode)));
	_trigger->addAction(TriggerActionPtr(new TriggerActionCallback(this)));
	_trigger->enable();
	enableUpdate();
}
