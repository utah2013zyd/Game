...
BOOST_AUTO_TEST_SUITE( LuaManager_ts )

struct SYSTEM {
	SYSTEM() {
		using namespace Orz;
		system.reset(new SystemList<boost::mpl::list<SimpleLogManager, SimpleTimerManager, OgreGraphicsManager, LuaManager, OISInputManager> >());
...
BOOST_FIXTURE_TEST_CASE(LUA, SYSTEM)
{
