#ifndef __Orz_FpfSvr_Common_h__
#define __Orz_FpfSvr_Common_h__
...
//! 将数字前面补给定数量的0
ORZ_PLUS_INLINE NumStr convert_num(uint32 _count, const NumStr& _num)
{
	if (_num.empty())
		return NumStr();
...
// 分解被指定字符分割的packet
ORZ_PLUS_INLINE int32 separateStr(const char* _str, uint32 _size, const char& _spt, std::vector<PacketData>& _r)
{
	if (!_str || _size == 0)
		return 1;
